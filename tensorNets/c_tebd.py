...
def run_TEBD_2(psi, U_bonds, Ubonds_dt_half, N_steps, chi_max, eps):
    ...
    assert len(U_bonds) == Nbonds
    for n in range(N_steps):
        for k, bonds in zip([0, 1, 0], [Ubonds_dt_half, U_bonds, Ubonds_dt_half]):  # even dt/2, odd dt, even dt/2
            for i_bond in range(k, Nbonds, 2):
                update_bond(psi, i_bond, bonds[i_bond], chi_max, eps)
    # done
